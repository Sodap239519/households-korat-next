    ...
    <template v-else-if="group">
      ...
      <div class="relative bg-gradient-to-br from-violet-600 via-violet-700 to-fuchsia-700 text-white overflow-hidden">
        ...
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div class="flex flex-col sm:flex-row items-center sm:items-start gap-5">
            ...
            <div class="shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border-2 border-white/40 shadow-xl overflow-hidden bg-white/20 backdrop-blur flex items-center justify-center">
              <!-- ร้านค้าย่อย: รูปโปรไฟล์ร้านเป็นหลัก (fallback → โลโก้กลุ่ม) -->
              <template v-if="seller">
                <img v-if="seller.avatar_path || group.logo_path"
                  :src="`/storage/${seller.avatar_path || group.logo_path}`"
                  :alt="seller.shop_name || seller.name" class="w-full h-full object-cover" />
                <span v-else class="text-white font-bold text-4xl">{{ (seller.shop_name || seller.name || '?').charAt(0) }}</span>
              </template>
              <!-- โซน: โลโก้กลุ่ม -->
              <template v-else>
            ...
            <div class="flex-1 text-center sm:text-left">
              <!-- ร้านค้าย่อย: ชื่อร้านเป็นหลัก + โซนเป็น secondary (คลิกดูโปรไฟล์โซนได้) -->
              <template v-if="seller">
                <h1 class="text-2xl sm:text-3xl font-bold drop-shadow">{{ seller.shop_name || seller.name }}</h1>
                <p class="mt-1 text-white/70 text-sm flex items-center gap-1 justify-center sm:justify-start">
                  <i class="fi fi-rr-store-alt text-xs"></i>
                  โซน:&nbsp;<RouterLink :to="`/shop/sellers/${group.slug}`"
                    class="text-white/90 hover:text-white hover:underline font-medium transition">{{ group.name }}</RouterLink>
                </p>
              </template>
              <!-- โซน: ชื่อกลุ่มเป็นหลัก -->
              <template v-else>
      ...
      <div class="max-w-5xl mx-auto px-4 sm:px-6 mt-5">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <!-- Rating -->
          <div class="box-card p-4 flex flex-col items-center gap-1 text-center">
            <div class="text-2xl font-bold text-amber-500">{{ group.avg_rating > 0 ? group.avg_rating : '—' }}</div>
          ...
          <button @click="toggleReviews"
            class="box-card p-4 flex flex-col items-center gap-1 text-center group hover:border-fuchsia-300 hover:bg-fuchsia-50/30 transition cursor-pointer w-full"
            :class="showReviews ? 'border-fuchsia-300 bg-fuchsia-50/30' : ''">
            ...
            <div class="text-[10px] text-fuchsia-400 opacity-0 group-hover:opacity-100 transition">{{ showReviews ? 'ซ่อน ↑' : 'ดูรีวิว →' }}</div>
          </button>

          <!-- Card 4: Zone view → ร้านค้าในโซน | Sub-store view → อำเภอร้านค้า -->
          <template v-if="!seller">
            <!-- Zone: ร้านค้าย่อยในโซน -->
            <button v-if="members.length > 0" @click="scrollToSection('store-members')"
              class="box-card p-4 flex flex-col items-center gap-1 text-center group hover:border-sky-300 hover:bg-sky-50/30 transition cursor-pointer w-full">
              <div class="text-2xl font-bold text-sky-600 group-hover:scale-110 transition-transform">{{ members.length }}</div>
              <div class="text-sky-400 text-lg"><i class="fi fi-rr-shop"></i></div>
              <div class="text-xs text-slate-400 group-hover:text-sky-600 transition">ร้านค้าในโซน</div>
              <div class="text-[10px] text-sky-400 opacity-0 group-hover:opacity-100 transition">ดูร้านค้า →</div>
            </button>
            <!-- Zone: ไม่มีร้านค้าย่อย → แสดงอำเภอ -->
            <button v-else @click="showDistricts = !showDistricts"
          ...
          <!-- Sub-store: อำเภอของร้านค้าย่อยนั้น (ไม่ใช่ district list ของ zone) -->
          <div v-else class="box-card p-4 flex flex-col items-center gap-1 text-center">
            <div class="text-xl font-bold text-emerald-600 truncate w-full text-center">
              {{ seller.district || seller.sub_district || '—' }}
            </div>
            <div class="text-emerald-400 text-lg"><i class="fi fi-rr-map-marker"></i></div>
            <div class="text-xs text-slate-400">อำเภอ</div>
          </div>
        </div>

        <!-- Districts panel (zone ที่ไม่มีร้านย่อย) -->
        <Transition name="slide-down">
          <div v-if="showDistricts && group.districts?.length && !seller && !members.length" class="mt-3 box-card p-4 border-emerald-100 bg-emerald-50/20">
      ...
      <!-- ===== ร้านค้าย่อยในโซน (zone view เท่านั้น) ===== -->
      <div v-if="!seller && members.length > 0" id="store-members" class="max-w-5xl mx-auto px-4 sm:px-6 mt-4">
        <div class="box-card p-4 sm:p-5">
          <h3 class="font-semibold text-slate-700 flex items-center gap-2 mb-4 text-sm">
            <i class="fi fi-rr-shop text-sky-500"></i>
            ร้านค้าในโซนนี้
            <span class="text-slate-400 font-normal">({{ members.length }} ร้าน)</span>
          </h3>
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            <RouterLink
              v-for="m in members" :key="m.id"
              :to="`/shop/sellers/${group.slug}?member=${m.id}`"
              class="flex flex-col items-center gap-2 p-3 rounded-2xl border border-slate-100 bg-slate-50 hover:border-violet-300 hover:bg-violet-50/40 transition group text-center">
              <!-- Avatar — รูปโปรไฟล์ของร้านแต่ละคนเป็นหลัก (fallback → โลโก้กลุ่ม) -->
              <div class="w-16 h-16 rounded-2xl overflow-hidden bg-gradient-to-br from-violet-400 to-fuchsia-500 flex items-center justify-center shrink-0 shadow-sm group-hover:scale-105 transition-transform">
                <img v-if="m.avatar_path || group.logo_path"
                  :src="`/storage/${m.avatar_path || group.logo_path}`"
                  :alt="m.shop_name || m.name"
                  class="w-full h-full object-cover"
                  @error="e => e.target.style.display='none'" />
                <span v-else class="text-white font-bold text-2xl">{{ (m.shop_name || m.name || '?').charAt(0) }}</span>
              </div>
              <!-- Name + product count -->
              <div class="min-w-0 w-full">
                <p class="font-semibold text-slate-700 text-sm truncate group-hover:text-violet-700 transition">{{ m.shop_name || m.name }}</p>
                <p class="text-xs text-slate-400 mt-0.5">{{ m.products_count }} สินค้า</p>
              </div>
            </RouterLink>
          </div>
        </div>
      ...
      <!-- ===== Review List (toggle) ===== -->
      <Transition name="slide-down">
        <div v-if="showReviews" id="store-reviews" class="max-w-5xl mx-auto px-4 sm:px-6 mt-3">
          <div class="box-card border-fuchsia-100 overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-fuchsia-50/30">
              <h4 class="text-sm font-semibold text-fuchsia-700 flex items-center gap-1.5">
                <i class="fi fi-rr-comment-alt"></i>
                รีวิวสินค้า{{ seller ? `ของ${seller.shop_name || seller.name}` : '' }}
              </h4>
              <span class="text-xs text-slate-400">{{ group.total_reviews || 0 }} รีวิว</span>
            </div>

            <!-- Loading -->
            <div v-if="reviewsLoading" class="px-4 py-8 flex justify-center text-slate-400">
              <i class="fi fi-rr-spinner animate-spin text-2xl"></i>
            </div>

            <!-- Empty -->
            <div v-else-if="!reviews.length" class="px-4 py-8 text-center text-slate-400 text-sm">
              <i class="fi fi-rr-comment-alt text-3xl mb-2 block text-slate-200"></i>
              ยังไม่มีรีวิวสินค้า
            </div>

            <!-- Review items -->
            <div v-else class="divide-y divide-slate-100">
              <div v-for="r in reviews" :key="r.id" class="px-4 py-4 flex gap-3">
                <!-- Reviewer avatar -->
                <div class="shrink-0 w-9 h-9 rounded-full overflow-hidden bg-gradient-to-br from-violet-400 to-fuchsia-500 flex items-center justify-center text-white text-sm font-bold">
                  <img v-if="r.user?.avatar_path" :src="`/storage/${r.user.avatar_path}`" :alt="r.user.name"
                    class="w-full h-full object-cover" @error="e => e.target.style.display='none'" />
                  <span v-else>{{ (r.user?.name || '?').charAt(0) }}</span>
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between gap-2 flex-wrap">
                    <span class="text-sm font-semibold text-slate-800">{{ r.user?.name || 'ผู้ใช้' }}</span>
                    <div class="flex items-center gap-0.5 text-amber-400 text-xs">
                      <i v-for="n in 5" :key="n" :class="n <= r.rating ? 'fi fi-sr-star' : 'fi fi-rr-star'"></i>
                    </div>
                  </div>
                  <!-- สินค้าที่รีวิว -->
                  <RouterLink v-if="r.product"
                    :to="`/shop/products/${r.product.slug}?tab=reviews`"
                    class="text-[11px] text-violet-600 hover:underline mt-0.5 block truncate">
                    <i class="fi fi-rr-box-open text-[10px]"></i> {{ r.product.name }}
                  </RouterLink>
                  <!-- เนื้อหารีวิว -->
                  <p v-if="r.comment" class="text-sm text-slate-600 mt-1 leading-relaxed line-clamp-3">{{ r.comment }}</p>
                  <!-- ตอบกลับจากร้านค้า -->
                  <div v-if="r.reply" class="mt-2 bg-violet-50 rounded-xl px-3 py-2 border border-violet-100">
                    <p class="text-[11px] font-semibold text-violet-700 flex items-center gap-1 mb-0.5">
                      <i class="fi fi-rr-shop text-[10px]"></i> ตอบกลับจากร้านค้า
                    </p>
            ...
            <!-- Load more -->
            <div v-if="reviewsMeta && reviewsMeta.current_page < reviewsMeta.last_page"
              class="px-4 py-3 border-t border-slate-100 text-center">
              <button @click="loadMoreReviews"
                :disabled="reviewsLoading"
                class="text-sm text-fuchsia-600 hover:text-fuchsia-800 font-medium flex items-center gap-1.5 mx-auto transition disabled:opacity-50">
                <i class="fi fi-rr-angle-down text-xs"></i> โหลดเพิ่มเติม
              </button>
            </div>
          </div>
        </div>
      ...
      <div v-if="seller
            ? (seller.products_description || seller.business_address || seller.district || group.contact_phone)
            : group.description"
        id="store-about" class="max-w-5xl mx-auto px-4 sm:px-6 mt-4">
        <div class="box-card p-4 sm:p-5">
          <h3 class="font-semibold text-slate-700 flex items-center gap-2 mb-2">
            <i class="fi fi-rr-info text-violet-500"></i> เกี่ยวกับร้าน
          </h3>

          <!-- ร้านค้าย่อย: ข้อมูลจากใบสมัคร -->
          <template v-if="seller">
            <p v-if="seller.products_description" class="text-slate-600 text-sm leading-relaxed">{{ seller.products_description }}</p>
            <div class="mt-3 pt-3 border-t border-slate-100 flex flex-col gap-1.5 text-sm text-slate-500">
              <span v-if="seller.business_address || seller.district" class="flex items-start gap-2">
                <i class="fi fi-rr-marker text-violet-400 mt-0.5 shrink-0"></i>
                <span>{{ [seller.business_address, seller.sub_district, seller.district].filter(Boolean).join(' ') }}</span>
              </span>
      ...
      <div id="store-products" class="max-w-5xl mx-auto px-4 sm:px-6 py-5">
        <div class="flex items-center justify-between mb-4">
          <h2 class="font-bold text-slate-800 flex items-center gap-2">
            <i class="fi fi-rr-grid text-violet-500"></i>
            สินค้าของ{{ seller ? (seller.shop_name || seller.name) : group.name }}
...
const seller        = ref(null) // ร้านค้าย่อย (เมื่อ ?member=user_id)
const members       = ref([])   // รายชื่อร้านค้าย่อยในโซน (zone view เท่านั้น)
...
async function loadReviews(page = 1) {
  ...
  try {
    ...
    if (route.query.member) params.member = route.query.member
    const { data } = await api.get(`/shop/sellers/${route.params.slug}/reviews`, { params })
    if (page === 1) {
      reviews.value = data.data
    } else {
      reviews.value.push(...data.data)
    }
    reviewsMeta.value = { current_page: data.current_page, last_page: data.last_page }
  } catch { /* ignore */ } finally { reviewsLoading.value = false }
}

function toggleReviews() {
  showReviews.value = !showReviews.value
  if (showReviews.value && !reviews.value.length) {
    loadReviews(1)
  }
  if (showReviews.value) {
    setTimeout(() => document.getElementById('store-reviews')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 120)
  }
}

function loadMoreReviews() {
  if (reviewsMeta.value) loadReviews(reviewsMeta.value.current_page + 1)
}
...
async function load(page = 1) {
  ...
  try {
    const params = { page }
    if (route.query.member) params.member = route.query.member
    const { data } = await api.get(`/shop/sellers/${route.params.slug}`, { params })
...
async function startChat() {
  try {
    const params = seller.value ? `?member=${seller.value.id}` : ''
    const { data } = await api.post(`/shop/chat/start/${group.value.slug}${params}`)
...
// เวลา navigate ระหว่าง zone ↔ member sub-store ให้โหลดข้อมูลใหม่
watch(() => [route.params.slug, route.query.member], () => {
