<template>
  <div class="p-3 sm:p-6 space-y-4 sm:space-y-5">
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <Button v-if="canCreateFollowup" label="เพิ่มการติดตาม" icon="fi fi-rr-plus" @click="openCreate" />
    </div>
...
const { canCreateFollowup, canActInDistrict } = useAuth()
function canActOn(row) {
  return canActInDistrict(row.allocation?.quota?.district)
}
