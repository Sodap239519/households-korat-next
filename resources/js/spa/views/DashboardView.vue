...
const incomeChart = computed(() => {
  ...
  return {
    ...
    datasets: [
      { label: 'รายได้',  data: o.value.byDistrict.map(d => Number(d.total_income)),  borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', tension: 0.35, fill: true },
      { label: 'รายจ่าย', data: o.value.byDistrict.map(d => Number(d.total_expense)), borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.1)', tension: 0.35, fill: true },
      { label: 'หนี้สิน', data: o.value.byDistrict.map(d => Number(d.total_debt)),    borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', tension: 0.35, fill: true },
    ],
  }
})
...
const channelChart = computed(() => {
  ...
  return {
    ...
    datasets: [
      { label: 'จำนวนครั้ง', data: t.value.byChannel.map(c => c.count),                              backgroundColor: '#a78bfa', borderRadius: 6, yAxisID: 'y' },
      { label: 'รายได้ (พันบ.)', data: t.value.byChannel.map(c => Number(c.total_revenue || 0) / 1000), backgroundColor: '#d946ef', borderRadius: 6, yAxisID: 'y' },
    ],
  }
})
